export default async function handler(req, res) {
  ...
  // Tentativa 1: generateContent com gemini-2.5-flash
  try {
    const url1 = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${geminiKey}`;
    ...
    if (r1.ok) {
      ...
      erros.push("T1: ok mas sem imagem");
    } else {
      erros.push("T1: " + (d1?.error?.message || JSON.stringify(d1).substring(0, 200)));
    }
  } catch (e) {
  ...
  try {
    const url2 = "https://generativelanguage.googleapis.com/v1beta/interactions";
    ...
    if (r2.ok) {
      ...
      erros.push("T2: ok mas sem imagem");
    } else {
      erros.push("T2: " + (d2?.error?.message || JSON.stringify(d2).substring(0, 200)));
    }
  } catch (e) {
    erros.push("T2: " + e.message);
  }

  return res.status(500).json({ error: "Falhou. Detalhes: " + erros.join(" | ") });
}
