...
export default function Landing() {
  ...
  return <main className="gl-site">
    ...
    <section id="experiencia" className="gl-story"><aside>{chapters.map((chapter, index) => <button className={active === index ? 'active' : ''} key={chapter[0]} onClick={() => refs.current[index]?.scrollIntoView({ behavior: 'smooth' })}><b>{chapter[0]}</b>{chapter[1]}</button>)}</aside>{chapters.map((chapter, index) => <section className={`gl-chapter c${index}`} key={chapter[0]} ref={node => refs.current[index] = node}><div className="gl-copy"><span>CAPÍTULO {chapter[0]} / {chapter[1]}</span><h2>{chapter[2]}</h2><p>{chapter[3]}</p></div><Visual index={index}/></section>)}</section>
